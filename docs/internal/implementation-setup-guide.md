...
| `app`              | Flask integration service plus built React config UI                  |
...
The current app container `CMD` starts Gunicorn directly; it does not run Alembic automatically.
...
The compose file overrides `DATABASE_URL` for the `app` service when using only `docker-compose.yml`:
...
3. Stores the OpenEMR registration metadata and Zoom credentials.
4. Validates Zoom credentials by requesting a Zoom Server-to-Server OAuth token.
5. Generates a deterministic `tenant_id` from Zoom account ID and client ID.
...
4. Enable or the dynamically registered OpenEMR client in OpenEMR.

- Navigate to OpenEMR login page in a browser
- Log in with `OPENEMR_ADMIN_USER` and `OPENEMR_ADMIN_PASS`
- Navigate to Admin -> System -> API Clients
- Find the newly registered app in Client Registrations. Client Name: **Zoomly Bridge - `Zoom Account ID`**
- Click the blue `Edit` button to edit the regisrations and then the `Enable Client` button in the top right of the registration window

5. Re-verify the registration in the config UI.

6. Configure appointment type filters.
...
5. OpenEMR verification succeeds after the dynamic client is enabled.
...
10. Deleting a linked appointment sends `appointment.deleted` and removes the local meeting record.
