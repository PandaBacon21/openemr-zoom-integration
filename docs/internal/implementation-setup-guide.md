...
- Outbound click-to-dial does not preload any cache. After ZCC accepts the initiate-call request it echoes an RC3 with `ContactType=Outgoing`, which Zoomly turns into an `outbound_call` SSE event → a small "Calling…" confirmation modal for the dialing agent (no chart navigation). Click-to-dial is invoked only from the patient Demographics contact section.
