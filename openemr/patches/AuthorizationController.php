...
class AuthorizationController
{
    ...
    public function clientRegisteredDetails(HttpRestRequest $request): ResponseInterface
    {
        ...
        try {
            ...
            // BUG: if ($client['registration_access_token'] !== $token) { 
    ...
    public function clientDelete(HttpRestRequest $request): ResponseInterface
    {
        $response = $this->createServerResponse();
        ...
        try {
            ...
            $uri_path = substr((string) $request->server->get('PATH_INFO'), $pos + 8);
            $client = sqlQuery("SELECT * FROM `oauth_clients` WHERE `registration_uri_path` = ?", [$uri_path]);
            // If client not found: 401, not 403 per FRC 7592 §2.3
            if (!$client) {
                throw new OAuthServerException('Invalid client', 0, 'invalid_client', Response::HTTP_UNAUTHORIZED);
            }
            // (verify the actual table/column names against OpenEMR's schema first)
            if ($client['registration_token'] !== $token) {
                throw new OAuthServerException('Invalid registration token', 0, 'invalid_request', Response::HTTP_FORBIDDEN);
            }
            // Delete the client row and clean up trusted user:
            sqlStatement("DELETE FROM `oauth_trusted_user` WHERE `client_id` = ?", [$client['client_id']]);
            sqlStatement("DELETE FROM `oauth_clients` WHERE `registration_uri_path` = ?", [$uri_path]);

            $this->getSystemLogger()->debug("clientDelete: removed client", ['uri_path' => $uri_path]);
            $this->session->invalidate();
