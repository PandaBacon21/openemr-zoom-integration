...
function fetch_Patient_Tracker_Events($from_date, $to_date, $provider_id = null, $facility_id = null, $form_apptstatus = null, $form_apptcat = null, $form_patient_name = null, $form_patient_id = null)
{
    ...
    // Zoomly patch: enrich each event with pc_website (Zoom host start URL).
    // One batched lookup keyed by pc_eid; fetchAppointments does not return pc_website.
    if (!empty($events)) {
        $eids = [];
        ...
        if (!empty($eids)) {
            $eid_list = array_keys($eids);
            $placeholders = implode(',', array_fill(0, count($eid_list), '?'));
            $res = sqlStatement(
                "SELECT pc_eid, pc_website FROM openemr_postcalendar_events WHERE pc_eid IN ($placeholders)",
                $eid_list
            );
            $url_by_eid = [];
            while ($row = sqlFetchArray($res)) {
                $url_by_eid[$row['pc_eid']] = $row['pc_website'];
            }
            foreach ($events as $k => $ev) {
                $eid = $ev['pc_eid'] ?? null;
                $events[$k]['pc_website'] = ($eid !== null && isset($url_by_eid[$eid])) ? $url_by_eid[$eid] : '';
            }
        }
    }
