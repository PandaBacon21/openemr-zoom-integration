...
def _create_mapping(account: ZoomAccount, *, npi: str, is_active: bool = True) -> UserMapping:
    mapping = UserMapping(
        zoom_account_id=account.account_id,
        openemr_fhir_id=f"fhir-{npi}",
        openemr_provider_npi=npi,
        openemr_user_id=f"id-{npi}",
...
def test_create_user_mapping_rejects_basic_zoom_license(app):
    with app.app_context():
        with pytest.raises(ValueError, match="Basic \\(free\\) license"):
            providers._create_user_mapping(
...
def test_create_user_mapping_requires_active_registration(app):
    with app.app_context():
...
def test_create_user_mapping_rejects_duplicate_npi_for_account(app):
    with app.app_context():
...
def test_create_user_mapping_allows_replacing_inactive_mapping(app):
    with app.app_context():
        account = _create_account("acct-1", is_active=True)
        _create_mapping(account, npi="1234567890", is_active=False)

        mapping = providers._create_user_mapping(
            zoom_account_id="acct-1",
            openemr_fhir_id="pract-2",
            openemr_provider_npi="1234567890",
            openemr_user_id=10,
        ...
        rows = UserMapping.query.filter_by(zoom_account_id=account.account_id).all()

    assert mapping.openemr_provider_npi == "1234567890"
    assert mapping.openemr_user_id == "10"
    assert mapping.zoom_user_email == "new@example.com"
    assert len(rows) == 2


def test_create_user_mapping_persists_zoom_user_timezone(app):
    """The IANA timezone string from the picked Zoom user's profile must
    land on UserMapping.zoom_user_timezone so meeting creation can use
...
def test_create_user_mapping_allows_null_zoom_user_timezone(app):
...
def test_get_user_mappings_returns_only_active_for_account(app):
    with app.app_context():
        ...
        result = providers._get_user_mappings("acct-1")

    assert [m.id for m in result] == [active.id]


def test_get_user_mappings_requires_active_registration(app):
    with app.app_context():
        with pytest.raises(ValueError, match="No active registration found for account missing"):
            providers._get_user_mappings("missing")


def test_delete_user_mapping_deletes_matching_mapping(app):
    with app.app_context():
        account = _create_account("acct-1", is_active=True)
        mapping = _create_mapping(account, npi="1234567890", is_active=True)

        providers._delete_user_mapping("acct-1", mapping.openemr_user_id)
        deleted = UserMapping.query.filter_by(id=mapping.id).first()

    assert deleted is None


def test_delete_user_mapping_raises_when_not_found(app):
    with app.app_context():
        _create_account("acct-1", is_active=True)
        with pytest.raises(ValueError, match="No active mapping found with NPI 999"):
            providers._delete_user_mapping("acct-1", "999")
