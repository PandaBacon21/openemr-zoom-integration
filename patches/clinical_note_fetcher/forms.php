...
<nav>
    ...
    <script>
        ...
        function retrieveZoomNote(encounterNumber, formId) {
            var btn = event.currentTarget;
            var status = document.getElementById('zoom-note-status-' + formId);

            btn.disabled = true;
            status.style.color = '#666';
            status.textContent = 'Fetching\u2026';

            fetch(window.location.origin + '/interface/patient_file/encounter/fetch_zoom_note.php?encounter=' + encounterNumber)
            .then(function(response) {
                if (response.ok) {
                    // Note written to DB — reload the encounter page to show updated forms
                    location.reload();
                } else {
                    return response.json().then(function(data) {
                        status.style.color = 'red';
                        status.textContent = data.error || 'Failed to retrieve note.';
                        btn.disabled = false;
                    });
                }
            })
            .catch(function(err) {
                status.style.color = 'red';
                status.textContent = 'Request failed \u2014 check bridge connection.';
                btn.disabled = false;
                console.error('retrieveZoomNote error:', err);
            });
        }
